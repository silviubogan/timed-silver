    ...
    public class Markdown : DependencyObject
    {
        ...
        private Inline ImageInlineEvaluator(Match match)
        {
            ...
            try
            {
                ...
                {
                    bool useSystemPathSeparator = true;
                    if (useSystemPathSeparator)
                    {
                        url = System.IO.Path.Combine(Environment.CurrentDirectory,
                            AssetPathRoot ?? string.Empty, url);
                        url = url.Replace("/", "\\");
                        url = url.Replace("%20", " ");
                    }
                    else
                    {
                        url = (AssetPathRoot ?? string.Empty) + "/" + url;
                    }
                }
